...
gets a chance to fail in a way that's hard to read.
    ...
    python tools/ue2g_diagnose.py <export_dir> [--godot-project <dir>] [-o report.txt]

<export_dir> is the folder holding models/ + textures/ + level_layout.json.
Requires nothing but Python -- no Unreal, no Godot, no PIL -- so it can be run
on a machine that has only the exported files, and its output is a single text
file that fully describes the state of an export.
...
def audit_layout(rep, export_dir):
    """Reads level_layout.json: actor/mesh counts, material scalars, transforms."""
    rep.section("Layout (level_layout.json)")
    path = os.path.join(export_dir, "level_layout.json")
    if not os.path.isfile(path):
        rep.error("level_layout.json is missing -- the layout export did not run. "
                  "It is a SEPARATE action from the mesh export.")
        return None
...
def audit_models(rep, export_dir, layout):
    """Reads every .gltf: image URIs, baked-texture leftovers, missing targets."""
    rep.section("Models (models/*.gltf)")
    models_dir = os.path.join(export_dir, "models")
    if not os.path.isdir(models_dir):
        rep.error("models/ folder is missing -- the mesh export did not run. "
                  "It is a SEPARATE action from the layout export.")
        return
...
def audit_textures(rep, export_dir):
    """Sizes and dimensions -- predicts the Godot import segfault before it happens."""
    rep.section("Textures (textures/*)")
    tex_dir = os.path.join(export_dir, "textures")
    if not os.path.isdir(tex_dir):
        rep.warn("textures/ folder is missing -- models will import untextured.")
        return
    ...
    if total > TOTAL_TEXTURE_BYTES_WARN or len(oversized) > 20:
        rep.warn("This texture set is large enough to crash Godot's importer: "
                 ...
                 "toolchain bug. Set 'Max Texture Resolution' in the exporter "
                 "(e.g. 2048), or cap it in Godot via Import defaults "
                 "(process/size_limit).")
    else:
...
def main():
    ...
    rep.fact("export folder", os.path.abspath(args.export_dir))
    rep.fact("python", sys.version.split()[0])

    layout = audit_layout(rep, args.export_dir)
    audit_models(rep, args.export_dir, layout)
    audit_textures(rep, args.export_dir)
    if args.godot_project:
        audit_godot_project(rep, args.godot_project)

    text = rep.render()
    print(text)

    out_path = args.out or os.path.join(args.export_dir, "ue2g_report.txt")
    try:
        ...
        print("\nCould not write report to %s (%s)" % (out_path, e))
